...
## Iteration 1 — 2026-09-07 — ed90763

⚠ UNDERSAMPLED (n=0) — tier `random` is below the 20,000-run floor set by the gate.
⚠ UNDERSAMPLED (n=0) — tier `greedy` is below the 20,000-run floor set by the gate.
⚠ UNDERSAMPLED (n=0) — tier `oracle` is below the 20,000-run floor set by the gate.

**GATE FORCED TO FAIL: a gate passed on thin data is not passed.**

**NOT MEASURED (1): G1.5** — no samples reached the gate for these. They are counted as FAIL: an unmeasured threshold is not a met threshold.

**MARGINAL (5): G1.1, G1.2, G1.4, G1.5, G1.6** — the 95% Wilson interval straddles the threshold, so the verdict on these rows would flip inside sampling noise. Treat as unproven in either direction.

| tier | runs | win rate | 95% CI (Wilson) | median decisions | median win time |
|---|---:|---:|---|---:|---:|
| `random` ⚠ | 0 | 0.00% | [0.00%, 100.00%] | 0 | n/a |
| `greedy` ⚠ | 0 | 0.00% | [0.00%, 100.00%] | 0 | n/a |
| `planner` | 20,000 | 58.61% | [57.93%, 59.29%] | 255 | 24.3 min |
| `oracle` ⚠ | 0 | 0.00% | [0.00%, 100.00%] | 0 | n/a |

```text
G1.1 0.00% PASS MARGINAL   threshold: < 2%   95% CI [0.00%, 100.00%]   random win rate
G1.2 0.00% FAIL MARGINAL   threshold: 8% – 25%   95% CI [0.00%, 100.00%]   off by 100.0%   greedy win rate
G1.3 58.61% PASS   threshold: 40% – 60%   95% CI [57.93%, 59.29%]   planner win rate
G1.4 0.00% FAIL MARGINAL   threshold: 65% – 88%   95% CI [0.00%, 100.00%]   off by 100.0%   oracle win rate
G1.5 n/a (not measured) FAIL MARGINAL   threshold: ≥ 2.5×   95% CI [0.58×, ∞]   planner / greedy win-rate ratio
G1.6 -58.61 pp FAIL MARGINAL   threshold: ≥ 10 percentage points   95% CI [-59.29 pp, 41.39 pp]   off by 686.1%   oracle − planner win rate
G2.1 55.92% PASS   threshold: ≥ 25%   median score gain from optimal reorder
G2.2 65.96% PASS   threshold: ≥ 50%   rounds where optimal ordering changed
G2.3 62.73% PASS   threshold: ≥ 60%   non-commutative machine pairs
G3.1 71.20% FAIL   threshold: ≤ 55%   off by 29.5%   most-used archetype share of planner wins
G3.2 7 PASS   threshold: ≥ 5   archetypes appearing in ≥ 15% of wins
G3.3 0.971 PASS   threshold: ≥ 0.80   normalized Shannon entropy over archetype usage
...
GATE: FAIL (9 failing: G1.2, G1.4, G1.5, G1.6, G3.1, G4.2, G5.1, G5.2, G6.1)

Weakest metric: G1.6 — next loop targets this.
...
Strict dominance (oracle ≥ planner ≥ greedy ≥ random) on 0 shared seeds: **VIOLATED**

- `oracle - planner` = -58.61 pp
- `planner - greedy` = 58.61 pp
- `greedy - random` = 0.00 pp

**INVERSION — this is a bug in the bots, not a finding about the game:**
- oracle (0.00%) < planner (58.61%)
